...
{{{ step "import_recursive" }}}

Now the file looks like:

{{{ step "cat_configmap_post_recursive_import" }}}
...
{{{ step "cue_eval_configmap_prometheus" }}}
...
{{{ step "add_field_previous_template_def" }}}

Add a file with the component label to each directory:

{{{ step "add_file_component_label_each_dir" }}}

Format the CUE files:

{{{ step "format_the_files" }}}

Let's try again to see if it is fixed:

{{{ step "cue_eval_snapshot2" }}}
...
{{{ step "simplify_frontend" }}}

Check differences:

{{{ step "check_diffs_post_frontend_simplify" }}}

Two lines with annotations added, improving consistency.

{{{ step "cue_trim_simplify_frontend" }}}
...
{{{ step "cue_trim_and_check_kitchen" }}}
